...
function serviceCost(service) {
  if (typeof service?.billedCost === "number") {
    return service.billedCost;
  }
...
function serviceEffectiveCost(service) {
  if (typeof service?.effectiveCost === "number") {
    return service.effectiveCost;
  }
  if (
...
function renderStrengths(signals) {
  ...
  for (const f of projectFacts) {
    if (String(f.id ?? "").startsWith("memory_")) {
      continue;
    }
...
function renderDataGaps(signals) {
  ...
  if (observabilityGap) {
    lines.push(observabilityGap);
  }
  if (!signals.usage) {
    lines.push(`- ${missingUsageSentence(signals)}`);
  }

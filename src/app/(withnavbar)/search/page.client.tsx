...
export const SearchClientPage = ({ search }: { search: string }) => {
  const [data, { fetchNextPage, hasNextPage, isFetchingNextPage }] =
    trpc.products.searchProduct.useSuspenseInfiniteQuery(
      { search },
      {
    ...
    return (
      <div className="mx-auto flex max-w-7xl flex-col items-center justify-center px-4 py-20 text-center">
        ...
        <p className="mt-2 max-w-md text-muted-foreground text-sm">
          ...
          <span className="font-medium text-foreground">"{search}"</span>. Try
          checking for typos or searching for another category.
        </p>
      </div>
    );
  }

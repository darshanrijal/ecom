...
async function main() {
  ...
  const budgetLabel =
    budget === Number.POSITIVE_INFINITY ? "unlimited (all)" : String(budget);
  const gated = [
    ...gateable
    ...
    JSON.stringify(
      {
        ...
        budget: {
          maxCandidates: budget === Number.POSITIVE_INFINITY ? "all" : budget,
...
function resolveBudget(args) {
  ...
  if (trimmed === "all" || trimmed === "unlimited" || trimmed === "-1") {
    args.budgetSource = args.maxCandidatesArg == null ? "env" : "flag";

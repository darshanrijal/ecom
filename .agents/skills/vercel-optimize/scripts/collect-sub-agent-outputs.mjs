...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    const a = argv[i];
    if (a === "--manifest") {
      out.manifestPath = resolve(argv[++i]);
...
function readPreResolvedRecords(manifest) {
  if (!manifest || !Array.isArray(manifest.preResolvedRecords)) {
    return [];
  }
...
function normalizeOutput(value) {
  ...
  if (Array.isArray(unwrapped)) {
    return unwrapped.filter(isRecordObject);
  }
  if (isRecordObject(unwrapped)) {
    return [unwrapped];
  }
...
function inferCandidateRefFromFile(file, expected, recordCount) {
  if (recordCount !== 1 || expected.length === 0) {
    return null;
  }
  if (expected.length === 1) {
    return expected[0].candidateRef;
  }
  const name = basename(file);
  const matches = expected.filter((b) => {
    if (!b.group && b.index == null) {
      return false;
    }

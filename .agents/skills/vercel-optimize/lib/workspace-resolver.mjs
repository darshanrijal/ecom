...
async function expandParts(currentDir, parts) {
  ...
  if (head === "" || head === ".") {
    return await expandParts(currentDir, rest);
  }
  if (head === "*" || head === "**") {
    let entries = [];
    try {
...
function buildPackageLookup(p) {
  ...
  return {
    resolveSubpath(subpath) {
      ...
      if (exactHit) {
        return joinPackagePath(p.dir, exactHit);
      }
...
async function expandResolvedSpecifier(
  ...
  const add = (file) => {
    ...
    if (out.length > 0 && out.length - 1 >= opts.perSpecifierCap) {
      return false;
    }
    seen.add(file);
    out.push(file);
  ...
  async function expandPureBarrel(file, requestedNames, depth) {
    if (depth >= opts.pureBarrelDepth) {
      return;
    }
    if (barrelVisited.has(file)) {
      return;
    }
    barrelVisited.add(file);
    const text = await tryReadText(file);
    if (text == null || !isPureBarrel(text)) {
      return;
    }
  ...
  async function expandSuffixFanout(file, depth) {
    if (depth >= opts.suffixFanoutDepth) {
      return;
    }
    if (!isSuffixFanoutFile(file)) {
      return;
    }
    const visitKey = `${file}:${depth}`;
    if (fanoutVisited.has(visitKey)) {
      return;
    }
    ...
    for (const ref of extractModuleReferences(text)) {
      ...
      if (isSuffixFanoutFile(next)) {
        await expandSuffixFanout(next, depth + 1);
      }
    }
  }
}
...
function forwardRelevanceScore(ref, requestedNames, siblingCount) {
  ...
  if (ref.exportedNames) {
    for (const name of requestedNames) {
      if (ref.exportedNames.has(name)) {
        return 100;
      }
    }
  }
  if (specifierMatchesNames(ref.specifier, requestedNames)) {
    return 50;
  }

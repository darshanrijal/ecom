...
function isAccountScope(rec) {
  ...
  if (typeof ref === "string" && ref.startsWith("platform_")) {
    return true;
  }
  return false;
}
...
function scoreSpecificity(rec) {
  ...
  if (INLINE_CODE_RE.test(codeText)) {
    s += 0.2;
  }
...
function scoreGrounding(rec, ctx) {
  ...
  const fenceText = [rec.currentBehavior, rec.desiredBehavior]
    .filter((x) => typeof x === "string")
    .join("\n");
  if (CODE_FENCE_RE.test(fenceText)) {
    s += 0.25;
  }
  CODE_FENCE_RE.lastIndex = 0;
...
function scoreEvidenceAccount(rec) {
  ...
  if (typeof rec.o11ySignal === "string" && rec.o11ySignal.length > 0) {
    s += 0.1;
  }
  return Math.min(1, roundTo(s, 4));
}
